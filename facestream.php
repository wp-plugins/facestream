...
//error_reporting(1);
define ( 'BP_FACESTREAM_VERSION', '1.0.0' );
...
function facestream_addCheckbox() {
    ...
    facestream_checkFacebookAuth($user_id);
...
function facestream_getFacebook($user_id='') {
    ...
    if(get_site_option('facestream_user_settings_syncbp')==0){
        ...
        if(!$user_id){
            ...
            $user_data       = $wpdb->get_results($wpdb->prepare("SELECT * FROM $wpdb->users WHERE id=$user_id;"));
        ...
        if($max_reached==0){
            ...
            if (get_usermeta ( $user_id, 'facestream_session_key' )) {
                ...
                if(get_usermeta ( $user_id, 'facestream_synctoac' )==1){
                    ...
                    //login
                    $last_update = get_usermeta ( $user_id, 'facestream_lastupdate' );

                    //did i do an update this hour already?
                    if ($last_update != $cur_hour) {
                        ...
                        if ($items) {
                            foreach ( $items as $item ) {
                                ...
                                if($admin_filter!=""){
                                    $arrAdminFilters = explode(",",$admin_filter);
                                    if($arrAdminFilters){
                                        foreach ($arrAdminFilters as $filter){
                                            $pos_filter = strpos($item_text," ".$filter);
                                            if($pos_filter >0){
                                                if(get_site_option('facestream_filter_show')==0){
                                                    $item_text = str_replace($filter,"",strtolower($item_text));
                                                }
                                            }else{
                                                $filter_pass = 0;
                                            }
                                ...
                                if($admin_filterexplicit!="" && $filter_pass==1){
                                    $arrAdminFiltersExplicit = explode(",",$admin_filterexplicit);
                                    if($arrAdminFiltersExplicit){
                                        foreach ($arrAdminFiltersExplicit as $filter){
                                            $pos_filter = strpos(strtolower($item_text),$filter);
                                            if($pos_filter >0){
                                                $filter_pass = 0;
                                            }
                                        }
                                ...
                                if($user_filtergood!="" && $filter_pass==1){
                                    $arrGoodFilters = explode(",",$user_filtergood);
                                    if($arrGoodFilters){
                                        foreach ($arrGoodFilters as $filter){
                                            $pos_filter = strpos(strtolower($item_text),$filter);
                                            if($pos_filter >0){
                                                $filter_pass = 1;
                                            }else{
                                ...
                                if($user_filterbad!="" && $filter_pass==1){
                                    $arrBadFilters = explode(",",$user_filterbad);
                                    if($arrBadFilters){
                                        foreach ($arrBadFilters as $filter){
                                            $pos_filter = strpos(strtolower($item_text),$filter);
                                            if($pos_filter >0){
                                                $filter_pass = 0;
                                            }
                        ...
                        //update last update hour
                        update_usermeta ( ( int ) $user_id, 'facestream_lastupdate', attribute_escape ( date ( 'dmyH' ) ) );
                    }
                }
            }
        }
...
function facestream_settings_screen_content() {
    ...
    if(get_usermeta ( $user_id, 'facestream_session_key' )){
		...
		<a href="http://www.facebook.com/login.php?api_key=<?php echo get_site_option('facestream_api_key');?>&display=popup&extern=1&fbconnect=1&req_perms=offline_access,read_stream,publish_stream&return_session=1&v=1.0&next=<?php echo $bp->root_domain;?>/?type=facebook&fb_connect=1&cancel_url=<?php echo $bp->root_domain;?>/?type=facebook"><input type="button" value="<?php echo __( 'Authorize with facebook', 'facestream_lang' );?>"></a><br/><br/>
	...
	<?php } ?>
        ...
        echo '
        <a href="http://www.facebook.com/login.php?api_key='.get_site_option('facestream_api_key').'&display=popup&extern=1&fbconnect=1&req_perms=offline_access,read_stream,publish_stream&return_session=1&v=1.0&next='.$bp->root_domain.'/?type=facebook&fb_connect=1&cancel_url='.$bp->root_domain.'/?type=facebook">
            <input type="button" value="'.__( 'Authorize with facebook', 'facestream_lang' ).'">
        </a>
        ';
    }
}
...
add_action ( 'plugins_loaded', 'facestream_setup_nav' );
    ...
    <tr valign="top">
        ...
        <td><input type="text" name="facestream_application_secret" value="<?php echo get_site_option('facestream_application_secret'); ?>" size="50"/></td>
          ...
          <tr>
            ...
            <?php echo __( 'you can set the filter to "soccer", now only facebook items with "soccer" will be shown of the users".', 'facestream_lang' );?><br>
            <?php echo __( 'By comma seperating words you can set-up multiple filters.', 'facestream_lang' );?> (<?php echo __( 'No filter = all items.', 'facestream_lang' );?>)<br>
        ...
        <tr valign="top">
            <th scope="row"><?php echo __( 'Show filters.', 'facestream_lang' );?></th>
            <th>
...
##############################################
##                                          ##
##          check auth on facebook           ##
##                                          ##
##############################################
function facestream_checkFacebookAuth($user_id){

    //do we still have access on facebook? else empty user keys.
    $token = get_usermeta ( $user_id, 'facestream_session_key' );
    $secret = get_usermeta ( $user_id, 'facestream_userid' );
...
add_action('facestream_hourly_event', 'facestream_cron');
add_filter('cron_schedules','facestream_cron_schedules');

function facestream_activation() {
    wp_schedule_event(time(), 'every_five_minutes', 'facestream_hourly_event');
}

function facestream_cron_schedules() {
...
//on plugin deactivation clear cron
register_deactivation_hook(__FILE__, 'facestream_deactivation');

function facestream_deactivation() {
    wp_clear_scheduled_hook('facestream_hourly_event');
}
